...
function TaskCreatePage() {
  ...
  useEffect(() => {
    ...
    async function loadData() {
      try {
        setLoading(true)
        const [projectsPayload, chefsPayload] = await Promise.all([
          getProjects(),
          getChefsDeProjet(),
        ])
        ...
        if (active) {
          setError(getApiErrorMessage(loadError, 'Unable to load task form data.'))
        }
      } finally {
        if (active) {
          setLoading(false)
  ...
  async function handleSubmit(event) {
    ...
    try {
      ...
      if (task?.id && selectedDevelopers.length > 0) {
        await bulkAssignTaskDevelopers(task.id, {
      ...
      setError(getApiErrorMessage(saveError, 'Unable to create task.'))
    } finally {
      setSaving(false)
    }
  }
  ...
  return (
    <>
      <PageSection className="hero-panel">
        ...
        <h1>Create a task.</h1>
        <p className="lead">Create the task, choose the project, and assign a chef plus developers.</p>
      </PageSection>

      <section className="info-grid">
        <InfoCard wide>
          {loading ? <p className="feedback-message">Loading task form...</p> : null}
