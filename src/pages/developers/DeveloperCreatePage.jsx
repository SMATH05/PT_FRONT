...
import { createDeveloper } from '../../services/developerService.js'
...
function DeveloperCreatePage() {
  ...
  async function handleSubmit(event) {
    ...
    try {
      ...
      const developer = getEntity(response)
      navigate(`/developers/${developer?.id ?? ''}`)
    } catch (saveError) {
      setError(getApiErrorMessage(saveError, 'Unable to create developer.'))
    } finally {
      setSaving(false)
    }
  }
  ...
  return (
    <>
      <PageSection className="hero-panel">
        ...
        <p className="lead">Create the developer and optionally link them to a manager.</p>
      </PageSection>
